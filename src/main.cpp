...
#if defined(NDEBUG)
# error "Cryptoruble cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "CryptoRuble Signed Message:\n";
CHooks* hooks = InitHook(); //this adds namecoin hooks which allow splicing of code inside standart cryptoruble functions.
...
{
    // cryptoruble: return false if time is below block 10 000 - we have no PoS blocks below 10 000 in our official blockchain.
...
void ThreadScriptCheck() {
    RenameThread("cryptoruble-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fWriteNames)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        else
        {
            ...
            // cryptoruble: moved to here from CheckInputs(), because this check now depends on context
            if (tx.IsCoinStake())
            {
    ...
    // cryptoruble: collect valid name tx
    ...
    // cryptoruble: add names to nameindex.dat
    if (fWriteNames)
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, bool fProofOfStake, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    if (!Params().SkipProofOfWorkCheck())
    {
        ...
        {
            // this is needed only for cryptoruble official blockchain, because of mistake we made at the beginning
...
// cryptoruble: note, this function can process blocks without strict order
...
bool AcceptBlockHeader(const CBlockHeader& block, bool fProofOfStake, CValidationState& state, CBlockIndex** ppindex)
{
    ...
    // cryptoruble: set PoS flag for CBlockIndex. This should probably be done immediately after we added header to CBlockIndex (i.e., after or inside AddToBlockIndex),
...
{
    // Preliminary checks
    //bool checked = CheckBlock(*pblock, state); // cryptoruble: removed, since this check happens later in AcceptBlock function
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    // cryptoruble: set/unset network serialization mode for new clients
