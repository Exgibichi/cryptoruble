...
#else
    const char* pszModule = "cryptoruble";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Cryptoruble";
#else
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "cryptoruble.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "cryptorubled.pid"));
